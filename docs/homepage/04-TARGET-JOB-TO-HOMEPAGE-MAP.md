# 04 — Target Job → Homepage Map

**Prepared:** 2026-07-07 · **Status:** Core doc, complete
**Source:** `samyang job.pdf` (Manager, Customer Experience — Samyang America, Brea CA). Responsibilities and requirements quoted/paraphrased below are from that posting.

This is the acceptance test for the whole build. Every homepage chapter must prove at least one of these. If a chapter cannot be tied to a responsibility, it is decoration and should be cut.

## A. Responsibilities → where the homepage proves them

| Job responsibility (from posting) | Homepage chapter(s) | How it is proven |
|---|---|---|
...
## B. Requirements → signals the homepage sends
...
| Establish operational strategy, standards, escalation | The entire operating-model framing. |
| Analytical, continuous-improvement focus | Rankings methodology + Product Signals loop. |
| Cross-functional stakeholder management | Named collaborators per case. |
| Track record improving CSAT / service levels / fill rates | Command metrics + improvement loop (synthetic, labeled). |

## C. Honesty guardrails tied to the job

- Never present synthetic operational metrics (CSAT, fill rate, deduction counts, case volumes) as real Samyang data. Label every one.
- Never imply access to Samyang systems or extensive SAP SD experience. The system is **integration-ready / system-agnostic**.
- Never present retail review counts as an official sales or satisfaction ranking.
- Serious matters (allergic reaction, injury, foreign material, tampering, package-integrity, regulatory) route to specialist escalation; the system does not diagnose, minimize, or give medical advice.

## D. Coverage check

Every MVP chapter maps to ≥1 responsibility above. Chapters that primarily serve discovery (Portfolio Pulse, Flavor Explorer, Comparison Lab, Dossier) map to **product knowledge and customer guidance**, which underpins "analyze customer trends," "effective issue resolution," and "improve CX." No orphan chapters.
